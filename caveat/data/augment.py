...
class SequenceJitter(ScheduleAugment):
    ...
    def __call__(self, sequence):
        ...
        mask = sequence[:, 0] > 1
        if mask.sum() < 2:  # single activity sequences cannot be jittered
            return sequence
...
class SmallestJitter(ScheduleAugment):
    def __init__(self, jitter: float = 0.1):
        """Augment a sequence by adding jitter to the duration of the shortest activity.
        Note that jitter defines the maximum delay or advance of the activity duration
        as a proportion. But note that during normalisation of the total plan duration
        this can be exceeded.
    ...
    def __call__(self, sequence):
        ...
        mask = sequence[:, 0] > 1
        length = mask.sum()
        if length < 2:  # single activity sequences cannot be jittered
            return sequence
