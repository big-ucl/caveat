...
def test_run_conv(config_discrete_conv):
    run_command(config_discrete_conv, test=True)


def test_run_vae_lstm(config_vae_lstm):
    run_command(config_vae_lstm, test=True)


def test_run_c_lstm(config_c_lstm):
    run_command(config_c_lstm, test=True)


def test_run_cvae_lstm(config_cvae_lstm):
    run_command(config_cvae_lstm, test=True)


def test_batch_multi_model(batch_config):
    batch_command(batch_config, test=True)


def test_nrun(config_vae_lstm):
    nrun_command(config_vae_lstm, n=2, test=True)
...
def test_jrun(config_jvae_lstm):
    jrun_command(config=config_jvae_lstm, test=True)
